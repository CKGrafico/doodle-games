...
try { const { PetancaView } = await import('./render.js'); await document.fonts.ready; view = new PetancaView($('court')); installViews(view, 'petanca', clearInput, { active, charging: () => mouse?.charging }); $('start').disabled = false; $('start').textContent = 'ECHAMOS UNA PARTIDA'; requestAnimationFrame(animate); } catch (e) { fatal(e); }
