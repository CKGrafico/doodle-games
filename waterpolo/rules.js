...
export function classifyBall(ball){if(Math.abs(ball.z)>POOL.halfLength+POOL.ballRadius){const end=Math.sign(ball.z),attacking=end<0?0:1;if(Math.abs(ball.x)+POOL.ballRadius<POOL.goalHalfWidth&&ball.y+POOL.ballRadius<POOL.goalHeight&&ball.y>=POOL.ballRadius)return{type:'goal',team:attacking};return{type:'goal-throw',team:1-attacking};}if(Math.abs(ball.x)>POOL.halfWidth+POOL.ballRadius)return{type:'out'};return null;}
