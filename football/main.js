...
function hud(){
  ...
  $('callout').hidden=!playing||game.stage==='playing'||game.stage==='fulltime';$('callout-title').textContent=game.message;$('callout-detail').textContent=['kickoff','restart'].includes(game.stage)&&game.players[game.ball.owner]?.team===0?'Press Space or tap Pass':game.stage==='goal'?'Back to the centre circle':game.stage==='halftime'?'Your attack direction changes next half':'Play resumes shortly';
  const total=game.stats.possession.reduce((a,b)=>a+b,0);$('possession-label').textContent=`POSSESSION ${total?Math.round(game.stats.possession[0]/total*100):50}%`;$('player-number').textContent='#'+game.players[game.controlled].number;$('stamina').value=game.players[game.controlled].stamina;
  const ctx=$('minimap').getContext('2d');ctx.clearRect(0,0,180,120);ctx.strokeStyle='#9296b0';ctx.lineWidth=1;ctx.strokeRect(4,4,172,112);ctx.beginPath();ctx.moveTo(90,4);ctx.lineTo(90,116);ctx.stroke();ctx.beginPath();ctx.arc(90,60,14,0,Math.PI*2);ctx.stroke();
...
$('camera').onclick=()=>{if(!view)return;view.mode=view.mode==='broadcast'?'tactical':'broadcast';view.resize();$('camera').textContent='Camera: '+view.mode;};$('assist-toggle').onclick=()=>{game.assisted=!game.assisted;assist();};
