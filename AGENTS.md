...
First-person controls must be designed for that view. Never reproject an absolute mouse position to ground while rotating a precision camera with the resulting shot angle: that feedback loop made golf unusable. Reuse `shared/first-person.js`: relative horizontal aiming, Shift precision and charge locking for stationary games; mouse look, camera-relative movement and centre aiming for movement games. Pointer lock is opt-in, must have a visible exit hint and must clear input on loss. Preserve touch and other views. Tests must include actual input-to-camera-to-aim relationships.

The user specifically requested surf inspired by Kelly Slater's Pro Surfer (2002), and a new herding game inspired by Sheep (2000). Preserve original assets and levels. Surf must retain board momentum, climbing/descending speed, the chasing curl, safe barrel exits and balance, Special mode and Free surf. Sheep must retain autonomous flock behaviour, indirect dog control, calm versus panic, line-of-sight calls, actual obstacle routes, rescue quotas and complete-level tests. Include sheep in every future collection review; quality expectations apply beyond sports.
