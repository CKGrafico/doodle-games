...
2. Discover the current game folders from the homepage and repository. Never assume the current list of five is exhaustive. Read each game's controls, simulation, renderer, help, and tests. Compare the strongest parts of each, not just the most recently added implementation.
...
- Shared code: `shared/ink.js`, `shared/controls.js`, `shared/theme.css`. Fix common behavior here instead of introducing competing copies.
