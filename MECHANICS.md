...
Doodle District provides the action-game reference: immediate player agency, visible targets and consequences, responsive feedback, separate game/menu states, and explicit help. The user now requires top, third-person and first-person views in every sport. These are original sport-specific cameras, with mouse look and optional pointer lock for movement games.
...
Surf is an original three-wave arcade heat visiting Glass Bay, Point Break and Reef Run, with the best two scores counting. Cutbacks require three metres of committed traversal; held carving produces a snap. Barrels require at least 1.25 seconds inside a marked tube, active balance control and a safe exit to score. Charged bottom turns build speed, and right-lip launches allow spins and grabs, with extra airtime in marked sections. Release steering to straighten before landing. Distinct tricks increase combo value; repeating one trick reduces its contribution. Five seconds without a new trick banks a combo, while a wipeout loses pending points. There are no passive points for simply waiting. Between waves the player explicitly continues. Three session challenges and device-local personal bests provide replay goals. It does not claim official judging or ocean simulation.

Ski is a four-rider arcade race on three courses (780/1080/1380 m). Opponents use the same rider physics, ramp, rock, gate and boost logic. Gates build streaks and refill boost; alternative wide lines contain ramps or boost pickups. A charge near a ramp primes a larger jump for two seconds, otherwise it boosts speed. Spins and grabs reward clean landings with more boost. Missed gates slow the rider, falls cost physical recovery time, and there are no hidden time penalties. Rank is based on actual finish crossings, interpolated within a step. Gate and obstacle checks are swept. Both rides run at 120 Hz, preserve mouse, keyboard and touch alternatives, and distinguish surface steering from airborne rotation. These are original arcade rules, not official competition regulations.

## Three camera views in every game

First-person input details: golf/pool rotate shot angle using relative horizontal mouse deltas; curling/petanca use their bounded headings with sport-specific sensitivity. Shift reduces mouse sensitivity to 20%. Selected power/distance can be adjusted with the wheel; charging locks direction. Padel, football, water polo, pickleball and sheep rotate yaw and bounded pitch; WASD uses the displayed orientation and the centre reticle drives a bounded ground target, including a finite fallback when looking above the horizon. Optional Lock mouse removes screen-edge limits; ordinary relative motion still works when unavailable. The lock button never fires a gameplay action. Menus, blur, mode changes and lock loss cancel charges.

Surf now uses board heading and momentum, with gravity-like speed changes across the wave face. Neutral steering trims down the line; W explicitly trims, S carves or grabs. Mouse steering is relative to the screen centre, independent of camera ray intersections. A breaking curl chases slow sideways lines. Barrels accumulate points at risk while balancing, then award them only on a safe exit; falling loses them. Diverse tricks fill Special; E or the touch button activates eight seconds of double points and larger airs. Three-wave heats remain available, alongside endless Free surf. Old personal bests use a different storage key because scoring changed.

Sheep is an indirect-control herding puzzle. WASD/arrows and the analogue stick move the dog; Shift/full stick runs. Sheep separate, cohere and react to proximity, with no automatic attraction toward the goal. Hold click/Space/BARK and release for a directional bark: more power increases range, impulse and panic. E/WHISTLE gathers and calms nearby visible sheep, with renewable following; Q/GATE operates the level-three switch only nearby. Fence geometry blocks both bodies and calls. Water forces a bridge route; a moving tractor makes a direct crossing risky. Sheep reaching the pen score once. Meeting the quota permits ending early; all sheep give an extra star, and clean fast runs another. Timed and relaxed modes store separate best stars. Wrong routes and heavy barking can lose sheep or time; restarts and complete original levels are required.

`shared/cameras.js` owns the Top view / 3rd person / 1st person selector. Third person preserves each original external renderer. Top view uses an orthographic overview with aspect-aware bounds; climbing uses an overhead angle from above the wall so holds remain visible. First person uses the controlled athlete's eye position, or the throw/delivery/cue position in precision games. Surf and ski follow the rider's eye height without applying trick rotation to the camera.

The rendered camera must be the same camera used for ray aiming, camera-relative movement and projected labels. Changing modes clears input and cancels charge. The controlled character is hidden only while rendering first person and is restored afterward. The base camera is restored before each legacy render/resize so orthographic and perspective implementations cannot overwrite one another. Selection is stored per game on this device. First-person movement games use relative mouse look and centre aim; precision games adjust shot heading with relative horizontal motion. Mouse lock is optional and must release safely on menus, mode changes, blur and Escape.
